#include <am.h>
#include <remu.h>

#define AUDIO_FREQ_ADDR (AUDIO_ADDR + 0x00)
#define AUDIO_CHANNELS_ADDR (AUDIO_ADDR + 0x04)
#define AUDIO_SAMPLES_ADDR (AUDIO_ADDR + 0x08)
#define AUDIO_SBUF_SIZE_ADDR (AUDIO_ADDR + 0x0c)
#define AUDIO_INIT_ADDR (AUDIO_ADDR + 0x10)
#define AUDIO_COUNT_ADDR (AUDIO_ADDR + 0x14)
#define SBUF_SIZE 0x10000

void __am_audio_init() { outl(AUDIO_SBUF_SIZE_ADDR, SBUF_SIZE); }

void __am_audio_config(AM_AUDIO_CONFIG_T *cfg) {
  *cfg = (AM_AUDIO_CONFIG_T){
      .present = true,
      .bufsize = inl(AUDIO_SBUF_SIZE_ADDR),
  };
  // printf("audio: bufsize = %d\n", cfg->bufsize);
}

void __am_audio_ctrl(AM_AUDIO_CTRL_T *ctrl) {
  outl(AUDIO_FREQ_ADDR, ctrl->freq);
  outl(AUDIO_CHANNELS_ADDR, ctrl->channels);
  outl(AUDIO_SAMPLES_ADDR, ctrl->samples);
  outl(AUDIO_INIT_ADDR, 1);
}

void __am_audio_status(AM_AUDIO_STATUS_T *stat) {
  stat->count = inl(AUDIO_SBUF_SIZE_ADDR);
}

static int audio_ptr = 0;
void __am_audio_play(AM_AUDIO_PLAY_T *ctl) {
  int count = inl(AUDIO_COUNT_ADDR);
  int sbuf_size = inl(AUDIO_SBUF_SIZE_ADDR);
  // printf("sbuf_size = %d, count = %d\n", sbuf_size, count);
  while (sbuf_size - count < ctl->buf.end - ctl->buf.start) {
    count = inl(AUDIO_COUNT_ADDR);
  }
  // printf("am_audio: play %d bytes\n", ctl->buf.end - ctl->buf.start);

  for (int i = 0; i < ctl->buf.end - ctl->buf.start; i++) {
    outl(AUDIO_SBUF_ADDR + audio_ptr, *(uint8_t *)(ctl->buf.start + i));
    audio_ptr++;
    if (audio_ptr >= SBUF_SIZE) {
      // printf("am: audio_ptr = %d\n", audio_ptr);
      audio_ptr = 0;
    }
  }
  outl(AUDIO_COUNT_ADDR, count + (ctl->buf.end - ctl->buf.start));
}
