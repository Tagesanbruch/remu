...
void npu_matmul(int8_t *a, int8_t *b, int32_t *c, int m, int n, int k) {
  ...
  for (int n_start = 0; n_start < n; n_start += n_tile) {
    ...
    // Extract the current weight tile.
...
void npu_conv2d(int8_t *input, int8_t *weight, int32_t *output, int batch,
  ...
  // ============================================================================
  // Hardware helper wrappers
  // ============================================================================

  volatile uint32_t *const NPU_REG_BASE = (volatile uint32_t *)NPU_BASE;

  static inline void npu_reg_write(uint32_t reg_off, uint32_t val) {
    *(volatile uint32_t *)(NPU_BASE + reg_off) = val;
  }

  static inline uint32_t npu_reg_read(uint32_t reg_off) {
    return *(volatile uint32_t *)(NPU_BASE + reg_off);
  }
  ...
  void npu_hw_im2col(uint32_t src_offset, uint32_t dst_offset, int input_h,
    ...
    npu_wait();
    npu_reg_write(REG_IM2COL_SRC_OFF, src_offset);
    npu_reg_write(REG_IM2COL_DST_OFF, dst_offset);
    npu_reg_write(REG_IM2COL_IN_HW, (input_h << 16) | (input_w & 0xFFFF));
    npu_reg_write(REG_IM2COL_KER_HW, (kernel_h << 16) | (kernel_w & 0xFFFF));
    npu_reg_write(REG_IM2COL_CHANNELS, channels);
    npu_reg_write(REG_IM2COL_STRIDE, (stride_h << 16) | (stride_w & 0xFFFF));
    npu_reg_write(REG_IM2COL_PADDING, (pad_top << 16) | (pad_left & 0xFFFF));
    npu_reg_write(REG_IM2COL_DILATION,
                  (dilation_h << 16) | (dilation_w & 0xFFFF));
    npu_reg_write(REG_IM2COL_CTRL, 1); // Start and wait for completion.
    npu_wait();
  }
  ...
  void npu_hw_transpose(uint32_t src_offset, uint32_t dst_offset, int rows,
    ...
    npu_wait();
    npu_reg_write(REG_TRANS_SRC_OFF, src_offset);
    npu_reg_write(REG_TRANS_DST_OFF, dst_offset);
    npu_reg_write(REG_TRANS_DIMS, (rows << 16) | (cols & 0xFFFF));
    npu_reg_write(REG_TRANS_ELEM_SIZE, elem_size);
    npu_reg_write(REG_TRANS_CTRL, 1); // Start.
    npu_wait();
  }
  ...
  void npu_gemm_with_offset(uint32_t m, uint32_t n, uint32_t k, uint32_t a_off,
    ...
    npu_wait();
    npu_reg_write(REG_GEMM_M, m);
    npu_reg_write(REG_GEMM_N, n);
    npu_reg_write(REG_GEMM_K, k);
    npu_reg_write(REG_GEMM_A_OFFSET, a_off);
    npu_reg_write(REG_GEMM_B_OFFSET, b_off);
    npu_reg_write(REG_GEMM_C_OFFSET, c_off);
    npu_reg_write(REG_GEMM_CTRL, 1); // Start.
    npu_wait();
  }

#define USE_NPU_ACCEL
  ...
  void npu_conv2d(int8_t *input, int8_t *weight, int32_t *output, int batch,
    ...
    for (int b = 0; b < batch; b++) {
...
#ifdef USE_NPU_ACCEL
      // Hardware path: transpose weights once, use software im2col for the
      // current tile, then run GEMM and write back the result.

      // Load weights [N, K] into weight SRAM.
      npu_dma_load_weight(weight, N * K);

      // Transpose weights in SRAM: [N, K] -> [K, N].
      npu_hw_transpose(0, 0, N, K, 1);

      // Process spatial positions in tiles.
      int tile_idx = 0;
      int total_tiles = (M_total + M_tile - 1) / M_tile;
      ...
      for (int m_start = 0; m_start < M_total; m_start += M_tile) {
        ...
        // Use software im2col to materialize the tile matrix.
        int row_start = (m_start / out_w) * stride;
        int row_end = ((m_end - 1) / out_w) * stride + kh;
        // Copy the tile matrix into feature SRAM.
        im2col_tile(in_batch, _conv2d_im2col_buf, in_c, in_h, in_w, kh, kw,
                    out_w, m_start, m_end, pad, stride);
        npu_dma_load_feature(_conv2d_im2col_buf, M_cur * K);

        // Run GEMM on the current tile.
        npu_gemm_with_offset(M_cur, N, K, 0, 0, 0);
        ...
        // Scatter the tile back to NCHW layout.
        for (int m_idx = 0; m_idx < M_cur; m_idx++) {
          int m = m_start + m_idx;
          int oh = m / out_w;
          int ow = m % out_w;
          for (int c = 0; c < N; c++) {
            out_batch[c * out_h * out_w + oh * out_w + ow] =
                _conv2d_gemm_out[m_idx * N + c];
          }
        }
      }
      ...
      // Transpose weights on CPU: [N, K] -> [K, N].
      ...
      for (int m_start = 0; m_start < M_total; m_start += M_tile) {
        tile_idx++;
        if (tile_idx % 10 == 0 || tile_idx == 1 || tile_idx == total_tiles) {
          printf("NPU Conv2D processing tile %d / %d (M_start=%d)\n", tile_idx,
                 total_tiles, m_start);
        }
        int m_end = m_start + M_tile;
        if (m_end > M_total)
        ...
        // Software im2col for the current tile.
        im2col_tile(in_batch, _conv2d_im2col_buf, in_c, in_h, in_w, kh, kw,
                    out_w, m_start, m_end, pad, stride);

        // DMA the tile matrix into feature SRAM.
        npu_dma_load_feature(_conv2d_im2col_buf, M_cur * K);
        ...
        // Store the GEMM result.
        npu_dma_store_output(_conv2d_gemm_out, M_cur * N * sizeof(int32_t));

        // Scatter the output tile back to NCHW layout.
        for (int m_idx = 0; m_idx < M_cur; m_idx++) {
          int m = m_start + m_idx;
          int oh = m / out_w;
          int ow = m % out_w;
    ...
    void npu_depthwise_conv2d(int8_t *input, int8_t *weight, int32_t *output,
      ...
      // Process channel by channel (depthwise = each channel processed
      // independently)
      for (int b = 0; b < batch; b++) {
        for (int c = 0; c < channels; c++) {
          int8_t *in_ch =
              input + b * (channels * in_h * in_w) + c * (in_h * in_w);
              ...
              for (int ky = 0; ky < kh; ky++) {
                for (int kx = 0; kx < kw; kx++) {
                  ...
                  if (ih >= 0 && ih < in_h && iw >= 0 && iw < in_w) {
                    sum += (int32_t)in_ch[ih * in_w + iw] *
                           (int32_t)wt_ch[ky * kw + kx];
                  }
                }
              }
    ...
    void npu_maxpool2d(int8_t *input, int8_t *output, int batch, int channels,
                       int in_h, int in_w, int kh, int kw, int stride,
                       int pad) {
    ...
    void npu_leaky_relu_elementwise(void *input, void *output, int len,
                                    int dtype, int32_t alpha_q16) {
      if (dtype == 0) { // int8
        int8_t *in = (int8_t *)input;
        int8_t *out = (int8_t *)output;
        for (int i = 0; i < len; i++) {
    ...
    void npu_clip_elementwise(void *input, void *output, int len, int dtype,
      ...
      if (dtype == 0) { // int8
        ...
        int8_t *out = (int8_t *)output;
        int8_t min8 =
            (int8_t)(min_val < -128 ? -128 : (min_val > 127 ? 127 : min_val));
        int8_t max8 =
            (int8_t)(max_val < -128 ? -128 : (max_val > 127 ? 127 : max_val));
        for (int i = 0; i < len; i++) {
          int8_t val = in[i];
          if (val < min8)
            val = min8;
    ...
    void npu_relu6_elementwise(void *input, void *output, int len, int dtype) {
      // ReLU6 = Clip(x, 0, 6)
      // For quantized int8, 6 may be scaled differently; assuming direct value
      // here
    ...
    // Simple requantize with right shift (for common case where scale is power
    // of 2)
    void npu_requantize_shift(int32_t *input, int8_t *output, int len,
                              int shift) {
      for (int i = 0; i < len; i++) {
