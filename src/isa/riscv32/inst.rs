...
use crate::common::{RemuState, SWord, Word};
    ...
    match dec.opcode {
        ...
        0b0101111 => {
            ...
            match (dec.funct7 >> 2, dec.funct3) {
                (0b00010, 0b010) => {
                    // LR.W
                    let val = match read_or_trap(cpu, addr, 4, pc) {
                        Some(v) => v,
                        None => return,
                    };
                    W!(cpu, dec.rd, val);
                ...
                (0b00011, 0b010) => {
                    // SC.W
                    let src2 = R!(cpu, dec.rs2);
                    if !write_or_trap(cpu, addr, 4, src2, pc) {
                        return;
                    }
                ...
                (0b00001, 0b010) => {
                    // AMOSWAP.W
                    let t = match read_or_trap(cpu, addr, 4, pc) {
                        Some(v) => v,
                        None => return,
                    };
                    let src2 = R!(cpu, dec.rs2);
                ...
                (0b00000, 0b010) => {
                    ...
                    let src2 = R!(cpu, dec.rs2);
                    if !write_or_trap(cpu, addr, 4, t.wrapping_add(src2), pc) {
                        return;
                    }
                    W!(cpu, dec.rd, t);
                }
                (0b00100, 0b010) => {
                    ...
                    let src2 = R!(cpu, dec.rs2);
                    if !write_or_trap(cpu, addr, 4, t ^ src2, pc) {
                        return;
                    }
                    W!(cpu, dec.rd, t);
                }
                ...
                (0b10000, 0b010) => {
                    ...
                    let src2 = R!(cpu, dec.rs2);
                    let min = if (t as SWord) < (src2 as SWord) {
                        t
                    } else {
                        src2
                    };
                    if !write_or_trap(cpu, addr, 4, min, pc) {
                        return;
                    }
                    W!(cpu, dec.rd, t);
                }
                (0b10100, 0b010) => {
                    ...
                    let src2 = R!(cpu, dec.rs2);
                    let max = if (t as SWord) > (src2 as SWord) {
                        t
                    } else {
                        src2
                    };
                    if !write_or_trap(cpu, addr, 4, max, pc) {
                        return;
                    }
                    W!(cpu, dec.rd, t);
                }
