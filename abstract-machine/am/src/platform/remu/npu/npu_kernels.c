...
void npu_conv2d(int8_t *input, int8_t *weight, int32_t *output, int batch,
  ...
  for (int b = 0; b < batch; b++) {
    ...
    for (int m_start = 0; m_start < M_total; m_start += M_tile) {
      ...
      im2col_tile(in_batch, _conv2d_im2col_buf, in_c, in_h, in_w, kh, kw, out_w,
                  m_start, m_end, pad, stride);
...
void npu_batchnorm(int8_t *input, int8_t *output, int32_t *gamma, int32_t *beta,
...
void npu_requantize(int32_t *input, int8_t *output, int len, int32_t scale_q16,
                    int8_t zero_point) {
  requantize_i32_to_i8(input, output, len, scale_q16, zero_point);
}

// Simple requantize with right shift (for common case where scale is power of
// 2)
