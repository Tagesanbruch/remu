...
void __am_gpu_fbdraw(AM_GPU_FBDRAW_T* ctl) {
  if(ctl->pixels != NULL){
    ...
    // for(int i = 0;i < N*N;i++){
    //   printf("pixel[i]=%d, ctl->pixels[i]=%d\n",pixel[i], (int*)ctl->pixels[i]);
    // }
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        // printf("x=%d, y=%d, i=%d, j=%d, w=%d, h=%d, W=%d, i + j * w=%d,(x + i) + (y+j) * W=%d, pixel[i + j * w]=%d\n", x, y, i, j, w, h, W, i + j * w,(x + i) + (y+j) * W, pixel[i + j * w]);
        // outl(FB_ADDR + (x + i) + (y+j) * W, pixel[i + j * w]);
