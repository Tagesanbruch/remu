...
pub fn init_sdl() {
    ...
    if display_disabled() {
        crate::Log!("SDL display disabled; running headless");
        return;
    }
    ...
    let sdl_context = match sdl2::init() {
        Ok(ctx) => ctx,
        Err(e) => {
            crate::Log!("SDL init failed; running headless: {}", e);
            return;
        }
    };
    let video_subsystem = match sdl_context.video() {
        Ok(video) => video,
        Err(e) => {
            crate::Log!("SDL video init failed; running headless: {}", e);
            return;
        }
    };
    ...
    let window = match video_subsystem
        .window("RISC-V32 REMU", width, height)
        .position_centered()
        .build()
    {
        Ok(window) => window,
        Err(e) => {
            crate::Log!("SDL window creation failed; running headless: {}", e);
            return;
        }
    };
    ...
    let mut canvas = match window.into_canvas().build() {
        Ok(canvas) => canvas,
        Err(e) => {
            crate::Log!("SDL canvas creation failed; running headless: {}", e);
            return;
        }
    };
    canvas.clear();
    ...
    let event_pump = match sdl_context.event_pump() {
        Ok(pump) => pump,
        Err(e) => {
            crate::Log!("SDL event pump creation failed; running headless: {}", e);
            return;
        }
    };
    ...
    let mut texture =
        match static_creator.create_texture_streaming(PixelFormatEnum::ARGB8888, width, height) {
            Ok(texture) => texture,
            Err(e) => {
                crate::Log!("SDL texture creation failed; running headless: {}", e);
                return;
            }
        };
...
pub fn update_screen(vmem: &[u8]) {
    ...
    if let Some(wrapper) = canvas_lock.as_mut() {
        ...
        if let Some(texture) = texture_lock.texture.as_mut() {
            ...
            if vmem.len() >= size {
                if texture
                    .update(None, &vmem[0..size], width as usize * 4)
                    .is_err()
                {
                    return;
                }
...
#[cfg(feature = "device")]
fn display_disabled() -> bool {
    std::env::var_os("REMU_NO_DISPLAY").is_some() || std::env::var_os("HEADLESS").is_some()
}
...
fn keymap(sc: sdl2::keyboard::Scancode) -> u32 {
    ...
    match sc {
        ...
        Num0 => AM_KEY_0,
        Minus => AM_KEY_MINUS,
        Equals => AM_KEY_EQUALS,
        Backspace => AM_KEY_BACKSPACE,
        Tab => AM_KEY_TAB,
        Q => AM_KEY_Q,
        W => AM_KEY_W,
        E => AM_KEY_E,
        ...
        O => AM_KEY_O,
        P => AM_KEY_P,
        LeftBracket => AM_KEY_LEFTBRACKET,
        RightBracket => AM_KEY_RIGHTBRACKET,
        Backslash => AM_KEY_BACKSLASH,
        ...
        K => AM_KEY_K,
        L => AM_KEY_L,
        Semicolon => AM_KEY_SEMICOLON,
        Apostrophe => AM_KEY_APOSTROPHE,
        ...
        M => AM_KEY_M,
        Comma => AM_KEY_COMMA,
        Period => AM_KEY_PERIOD,
        Slash => AM_KEY_SLASH,
        RShift => AM_KEY_RSHIFT,
        LCtrl => AM_KEY_LCTRL,
        Application => AM_KEY_APPLICATION,
        LAlt => AM_KEY_LALT,
        Space => AM_KEY_SPACE,
        RAlt => AM_KEY_RALT,
        RCtrl => AM_KEY_RCTRL,
        ...
        Left => AM_KEY_LEFT,
        Right => AM_KEY_RIGHT,
        Insert => AM_KEY_INSERT,
        Delete => AM_KEY_DELETE,
        Home => AM_KEY_HOME,
        End => AM_KEY_END,
        PageUp => AM_KEY_PAGEUP,
        PageDown => AM_KEY_PAGEDOWN,
        _ => AM_KEY_NONE,
    }
}
